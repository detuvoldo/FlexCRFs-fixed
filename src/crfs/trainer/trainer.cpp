...
void trainer::train(FILE * fout) {
	...
	*/	
	...
	printf("\tIteration elapsed: %lu seconds\n", e_iter_time);
	if (is_logging) {
	    fprintf(fout, "\tIteration elapsed: %lu seconds\n", e_iter_time);
	}
	...
	if (popt->evaluate_during_training) {
	    ...
	    e_iter_time -= s_iter_time;
	    // display the elapsed time
	    printf("\tTraining iteration elapsed (including testing & evaluation time): %lu seconds\n",
		    e_iter_time);
	    if (is_logging) {
		fprintf(fout, "\tTraining iteration elapsed (including testing & evaluation time): %lu seconds\n",
		    e_iter_time);
	    }    
	}
    ...
    printf("\nThe training process elapsed: %lu seconds\n\n", e_train_time);
    if (is_logging) {
	fprintf(fout, "\nThe training process elapsed: %lu seconds\n\n", e_train_time);
    }
